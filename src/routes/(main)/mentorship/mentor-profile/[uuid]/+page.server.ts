...
import type { PageServerLoad } from '../../../../../../.svelte-kit/types/src/routes/(main)/mentorship/mentor-profile/[uuid]/$types.js';
...
export const load: PageServerLoad = async ({ locals, cookies, params }) => {
	...
	const request = await fetch(
		`${PUBLIC_SERVER_URL}/api/mentees/requests/${mentorData.mentorUuid}`,
		{
			method: 'GET',
			headers: {
				Authorization: `Bearer ${cookies.get('access_token')}`
	...
	if (
		!request.ok ||
		!!myMentors.find((mentor: { uuid: string }) => mentor.uuid === mentorData.uuid)
	) {
		alreadyRequested = true;
		alreadyMyMentor = true;
	}

	if (!myMentors.find((mentor: { uuid: string }) => mentor.uuid === mentorData.mentorUuid)) {
