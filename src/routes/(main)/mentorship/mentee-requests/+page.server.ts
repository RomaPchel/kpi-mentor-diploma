...
export const actions = {
	approve: async ({ request, cookies }) => {
		...
		const response = await fetch(`${PUBLIC_SERVER_URL}/api/user/mentee-request/${uuid}/approve`, {
			method: 'POST',
			headers: {
	...
	reject: async ({ request, cookies }) => {
		...
		const response = await fetch(`${PUBLIC_SERVER_URL}/api/user/mentee-request/${uuid}/reject`, {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
				Authorization: `Bearer ${cookies.get('access_token')}`,
